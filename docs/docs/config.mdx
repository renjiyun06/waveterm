...
| app:fileworkspacekeybinding                                  | string   | Keybinding that toggles the transient file workspace (defaults to `Cmd:Shift:e`). Use a colon-separated description such as `Ctrl:Shift:e`; set it to an empty string to disable the shortcut. Changes apply without restarting Wave.                         |
...
| fileworkspace:opacity                                        | float64  | Opacity of the transient file workspace, from `0.2` to `1` (defaults to `0.9`). This does not affect magnified blocks.                                                                                                                                        |
| fileworkspace:width                                          | float64  | Width of the transient file workspace as a fraction of the terminal layout, from `0.4` to `1` (defaults to `1`).                                                                                                                                              |
| fileworkspace:height                                         | float64  | Height of the transient file workspace as a fraction of the terminal layout, from `0.3` to `1` (defaults to `0.78`).                                                                                                                                          |
