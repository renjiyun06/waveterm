...
Pressing <Kbd k="F11" /> enters full-screen mode and hides the top bar so the active tab can use the entire window. Press <Kbd k="F11" /> again to restore the top bar. An open Wave AI panel keeps its current state while the top-bar toggle is hidden.
...
Right-click a block header and select **Rename Block** to give that block a custom title. Block titles are saved with the layout and can be different for every block in a split. Clear the title in the rename dialog to restore the block's default title.
