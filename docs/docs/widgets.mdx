...
Use the mode switcher at the right side of the workbench header to choose Files, Terminal, or Browser. Each mode has its own live session: switching modes or hiding the workbench keeps file tabs and drafts, the terminal process and scrollback, and browser navigation intact. These transient sessions reset when Wave restarts. Links that request a new browser window are loaded in the same workbench browser instead of creating another block. External application protocols are blocked so websites cannot repeatedly launch native-app or app-store prompts.
...
Use the sliders button beside the mode switcher in the workbench header to adjust the workbench opacity, width, and height. The corresponding settings are `fileworkspace:opacity`, `fileworkspace:width`, and `fileworkspace:height`; they apply to the complete Files, Terminal, and Browser surfaces and are independent from magnified block settings. Width changes are committed when a pointer drag finishes so the settings popover stays stable while its slider is being adjusted.
