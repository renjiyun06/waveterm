...
Press <Kbd k="Cmd:Shift:e" /> to slide a temporary file workspace down over the current terminal layout. Press the shortcut again to close it. Change the shortcut with `app:fileworkspacekeybinding` in `settings.json`; changes apply immediately. The panel uses the focused block's connection and lets you switch connections from its header.
...
Use the sliders button in the explorer header to adjust the panel opacity, width, and height. The corresponding settings are `fileworkspace:opacity`, `fileworkspace:width`, and `fileworkspace:height`; they are independent from magnified block settings.
