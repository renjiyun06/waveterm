...
Use the mode switcher in the workbench header to choose Files, Terminal, or Browser. Each mode has its own live session: switching modes or hiding the workbench keeps file tabs and drafts, the terminal process and scrollback, and browser navigation intact. These transient sessions reset when Wave restarts. Links that request a new browser window are loaded in the same workbench browser instead of creating another block.
...
Use the sliders button in the Files explorer header to adjust the workbench opacity, width, and height. The corresponding settings are `fileworkspace:opacity`, `fileworkspace:width`, and `fileworkspace:height`; they apply to all three workbench modes and are independent from magnified block settings.
