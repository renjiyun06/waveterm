...
Folders added to the explorer are stored with the current Wave workspace and can be removed without deleting them from disk. Each connection has its own folder list. Git repositories show dirty folders and files, while the editor gutter marks added, modified, and deleted lines. Unsaved editor changes are shown separately from Git status.

The panel uses the `window:magnifiedblockopacity` setting for transparency.
